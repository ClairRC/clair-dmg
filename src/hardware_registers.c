...
void init_hw_registers() {
    ...
    */
    ...
    HW_REG(0x4C, 0x04, 0x04, 1); //KEY0 This should become read-only after the bootrom is unmapped
    HW_REG(0x4D, 0x81, 0x01, 1); //Bit 7 R/O,  CGB only
    HW_REG(0x4E, 0x00, 0x00, 0);
    HW_REG(0x4F, 0x01, 0x01, 1); //VRAM banks. Bit 0 is the only bit that isnt ignored
    ...
    HW_REG(0x68, 0xBF, 0xBF, 1);
    HW_REG(0x69, 0xFE, 0xFE, 1);
    HW_REG(0x6A, 0xFE, 0xFE, 1);
    HW_REG(0x6B, 0xFE, 0xFE, 1);
    HW_REG(0x6C, 0x01, 0x01, 1); //OPRI is R/W, but writes are disabled after bootrom
    HW_REG(0x6D, 0x00, 0x00, 0);
    HW_REG(0x6E, 0x00, 0x00, 0);
    HW_REG(0x6F, 0x00, 0x00, 0);
    HW_REG(0x70, 0x07, 0x07, 1); //WRAM bank switch. Bottom 3 bits are R/W
